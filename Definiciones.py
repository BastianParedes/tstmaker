...
import os
from random import choice, shuffle, uniform, randrange
import shutil
import time
import threading
import subprocess
...
Configuracion = eval(open('Configuracion.py', 'r', encoding='utf-8').read())

URL = 'https://tstmaker.herokuapp.com'
...
class New_document(pylatex.Document):
    '''
    Ya está configurado para usar lualatex
    Crea la carpeta temp si no está creada
    Usa subprocess.run que no abre cmd, espera a que termine el proceso antes de seguir, no se puede poner en modo silencioso (de hacerlo crashea el .exe)-
    '''
    def __init__(self, ruta_encabezado='', ruta_pie='', posicion_encabezado='c', posicion_pie='c'):
        super().__init__(documentclass='article', font_size='normalsize')
        for paquete in [r'\usepackage{ragged2e}', r'\usepackage{setspace}', r'\usepackage{longtable}', r'\usepackage{tabularx}', r'\usepackage{gensymb}', r'\usepackage{amsmath}', r'\usepackage{amssymb}', r'\usepackage{enumitem}', r'\usepackage{graphicx}', r'\usepackage{tikz}', r'\usepackage{tkz-euclide}', r'\usepackage{siunitx}', r'\usepackage{fourier}', r'\usetikzlibrary{fit, shapes.geometric, quotes, angles, through, intersections}', r'\usepackage{fancyhdr}']:
            self.preamble.append(pylatex.NoEscape(paquete))
        self.change_document_style('fancy')
        self.append(pylatex.NoEscape(r'\renewcommand{\headrulewidth}{0pt} \renewcommand{\footrulewidth}{0pt}'))#Quita una linea del encabezado y del pie de página
        #Pone en el encabezado
        if ruta_encabezado=='':
            self.append(pylatex.NoEscape(f'\\{posicion_encabezado}head{{}}'))
        else:
            self.append(pylatex.NoEscape(f'\\{posicion_encabezado}head{{\\includegraphics{{{ruta_encabezado}}}}}'))
        #Pone el pie de página, si no se pone esto, aparecerá el número de la página
        if ruta_pie=='':
            self.append(pylatex.NoEscape(f'\\{posicion_pie}foot{{}}'))
        else:
            self.append(pylatex.NoEscape(f'\\{posicion_pie}foot{{\\includegraphics{{{ruta_pie}}}}}'))

    def generate_pdf(self, name, generate_tex=True):
        #Crea la carpeta temp si no existe
        if not os.path.exists('temp'):
            os.mkdir('temp')
        #Genera el text
        if generate_tex:
            self.generate_tex('temp/'+ name)
        #Crea el pdf
        subprocess.run(f'lualatex -interaction=batchmode -output-directory="temp" "temp/{name}.tex"', shell=True)
